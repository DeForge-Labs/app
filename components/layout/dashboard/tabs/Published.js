      ...
      {view === "grid" && templates?.length > 0 && !isWorkflowInitializing && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mt-4">
          {filteredTemplates.map((workflow) => (
            <GridCard flow={workflow} key={workflow.id} type="template" />
          ))}
        </div>
      )}

      {view === "list" && templates?.length > 0 && !isWorkflowInitializing && (
        <div className="space-y-4 mt-4">
          {filteredTemplates.map((workflow) => (
            <ListCard flow={workflow} key={workflow.id} type="template" />
          ))}
        </div>
      )}
    </div>
